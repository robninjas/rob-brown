    ...
    void Update()
    {
        ...
        if (Time.time > nextSpawn)
        {
            ...
            for (int i = 0; i < playerController.currentLevel; i ++)
            {
                ...
                if (playerController.currentLevel >= 10)
                {
                    instance.transform.localScale = new Vector3(2, 2, 2);
                    instance.GetComponent<EnemyController>().enemyHealth = 40;
                    instance.GetComponent<MeshRenderer>().materials[0].color = new Color(55, 55, 55);
                }
            }
