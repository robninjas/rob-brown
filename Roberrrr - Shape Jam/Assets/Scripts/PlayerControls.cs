    ...
    void Update()
    {
        ...
        if ((Input.GetKey("space") || Input.GetKey("z")) && (Time.time > nextFire)) 
        {
            ...
            if (currentLevel >= 3)
            {
                ...
                Vector3 right2Offset = new Vector3(0.4f, 0, 0);
                Vector3 left2Offset = new Vector3(-0.4f, 0, 0);

                Instantiate(projectile, transform.position + rightOffset, transform.rotation);
                Instantiate(projectile, transform.position + new Vector3(0f, 1f, 0), transform.rotation);
                Instantiate(projectile, transform.position + leftOffset, transform.rotation);
                Instantiate(projectile, transform.position + right2Offset, transform.rotation);
                Instantiate(projectile, transform.position + left2Offset, transform.rotation);
            }
